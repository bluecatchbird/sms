...
async def getArticle(
    project_id: uuid.UUID, article_id: uuid.UUID, db: Session = Depends(get_db)
):
    article = (
...
async def getElement(
    ...
    element = (
        ...
        .join(models.Project, models.Project.id == project_id)
        .join(models.Article, models.Article.id == article_id)
        .filter(models.Element.id == element_id)
        .first()
    )
    if not element:
        raise HTTPException(
...
@app.get(
    "/project/{project_id}", response_model=schemas.ProjectDetailed, tags=["project"]
)
...
@app.delete(
    "/project/{project_id}",
    response_model=List[schemas.ProjectWithId],
    tags=["project"],
)
async def deleteProjectById(
    project: models.Project = Depends(getProject), db: Session = Depends(get_db)
):
    db.delete(project)
    db.commit()
    return await getAllProjects(db)
...
@app.post(
    "/project/{project_id}/article",
    response_model=schemas.ArticleDetailed,
    tags=["article"],
)
async def createNewArticle(
    name: str,
    project: models.Project = Depends(getProject),
    db: Session = Depends(get_db),
):
...
@app.get(
    "/project/{project_id}/article/{article_id}",
    response_model=schemas.ArticleDetailed,
    tags=["article"],
)
...
@app.delete(
    "/project/{project_id}/article/{article_id}",
    response_model=schemas.ProjectDetailed,
    tags=["article"],
)
async def deleteArticle(
    article: models.Article = Depends(getArticle), db: Session = Depends(get_db)
):
...
@app.post(
    "/project/{project_id}/article/{article_id}/element",
    response_model=schemas.ElementWithId,
    tags=["element"],
)
async def addElement(
...
@app.patch(
    "/project/{project_id}/article/{article_id}/element/{element_id}",
    response_model=schemas.ElementWithId,
    tags=["element"],
)
async def patchElement(
    new_element: schemas.Element,
...
async def deleteElement(
    element: models.Element = Depends(getElement), db: Session = Depends(get_db)
):
...
@app.patch(
    "/project/{project_id}/article/{article_id}/notes",
    response_model=schemas.ArticleDetailed,
    tags=["notes"],
)
...
@app.post(
    "/project/{project_id}/article/{article_id}/image",
    response_model=schemas.ArticleDetailed,
    tags=["image"],
)
async def addImage(
    ...
    new_image.storage_path = "/".join(
        ["upload_data", str(article.project_id), str(article.id), file_name]
    )
    db.commit()
...
@app.delete(
    "/project/{project_id}/article/{article_id}/image/{image_id}",
    response_model=schemas.ArticleDetailed,
    tags=["image"],
)
async def deleteImage(
    image: models.Image = Depends(getImage), db: Session = Depends(get_db)
):
